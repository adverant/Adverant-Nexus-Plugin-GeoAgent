/**
 * Usage Tracking Middleware for GeoAgent
 *
...
function detectOperation(req: Request): string {
  ...
  // GeoAgent API operations
  if (path.includes('/layers')) {
    if (method === 'POST') return 'layer_create';
    if (method === 'PUT') return 'layer_update';
    if (method === 'DELETE') return 'layer_delete';
    return 'layer_query';
  }
  if (path.includes('/features')) {
    if (method === 'POST') return 'feature_create';
    if (method === 'PUT') return 'feature_update';
    if (method === 'DELETE') return 'feature_delete';
    return 'feature_query';
  }
  if (path.includes('/spatial')) {
    if (path.includes('/intersect')) return 'spatial_intersect';
    if (path.includes('/buffer')) return 'spatial_buffer';
    if (path.includes('/nearest')) return 'spatial_nearest';
    if (path.includes('/within')) return 'spatial_within';
    return 'spatial_operation';
  }
  if (path.includes('/tracking')) return 'tracking_operation';
  if (path.includes('/geofencing')) return 'geofencing_operation';
  if (path.includes('/h3')) return 'h3_operation';
  if (path.includes('/ingestion')) return 'data_ingestion';
  if (path.includes('/jobs')) return 'job_operation';
  if (path.includes('/google')) {
    if (path.includes('/earth-engine')) return 'earth_engine';
    if (path.includes('/vertex-ai')) return 'vertex_ai';
    if (path.includes('/bigquery')) return 'bigquery_gis';
    return 'google_integration';
  }
  if (path.includes('/hypermodal')) {
    if (path.includes('/lidar')) return 'lidar_processing';
    if (path.includes('/spectral')) return 'spectral_analysis';
    if (path.includes('/sar')) return 'sar_processing';
    if (path.includes('/thermal')) return 'thermal_analysis';
    if (path.includes('/fusion')) return 'multimodal_fusion';
    return 'hypermodal_operation';
  }
...
async function sendUsageReport(report: UsageReport, retryCount = 0): Promise<void> {
  ...
  try {
    ...
    if (response.status === 204 || response.ok) {
      logger.info(`[Usage Tracking] Report sent: ${report.operation} (user: ${report.userId})`);
      return;
    }

    const errorBody = await response.text().catch(() => 'unknown');
    logger.warn(`Usage tracking error response: ${response.status} - ${errorBody}`);
    ...
    if (!errorMessage.includes('abort')) {
      logger.warn(`Failed to send usage report: ${errorMessage}`);
    }
...
export function usageTrackingMiddleware(
  ...
  res.on('finish', () => {
    ...
    logger.info(`[Usage Tracking] Sending report: ${operation} (user: ${userId}, plugin: ${report.pluginName || 'none'})`);

    if (USAGE_TRACKING_CONFIG.enableBatching) {
      queueReport(report);
    } else {
      sendUsageReport(report).catch((err) => {
        logger.error(`[Usage Tracking] Unexpected error: ${err instanceof Error ? err.message : String(err)}`);
      });
    }
  });
...
export async function flushPendingReports(): Promise<void> {
  logger.info(`Flushing pending usage reports: ${usageQueue.length}`);
  await flushBatch();
}
